...
def convert_file_dxf():
    ...
    errors, duplicate_color_errors, cad_color_errors = check_files_errors(get_config_file(), post)
    ...
    if not errors:
        layers = get_layers_table()
          ...
          errors_square() or errors_rectangle()) \
            ...
            get_errors_config_file() or file_empty(os.path.join(session['files_folder'], session['config_file'])) or
            get_errors_config_file_duplicate_elements())) \
    ...
    if request.method == "POST":
        ...
        # Load possible file errors
        errors, duplicate_color_errors, cad_color_errors = check_files_errors(get_dxf_configuration(layers), post)

        if duplicate_color_errors or cad_color_errors:
            return render_template('convert.html', title='Conversion DXF', form=form, layers=layers,
                                   dxf_filename=session['dxf_filename'],
                                   cad_version=cad_version, symbols=get_symbols(),
                                   cad_versions=app.config["CAD_VERSIONS"], errors=errors,
                                   duplicate_color_errors=duplicate_color_errors, cad_color_errors=cad_color_errors)

        # Generate the DXF and add it to the list of user generated in the session
        converted = generate_dxf(session['files_folder'], session['dxf_filename'], layers, cad_version)
        if converted:
            session['converted_files'].append({'folder': session['files_folder'], 'file': session['dxf_filename']})
    ...
    return render_template('convert.html', title='Conversion DXF', form=form, layers=layers,
                           dxf_filename=session['dxf_filename'],
                           cad_version=cad_version, symbols=get_symbols(), cad_versions=app.config["CAD_VERSIONS"],
                           errors=errors,
                           duplicate_color_errors=duplicate_color_errors, cad_color_errors=cad_color_errors)
